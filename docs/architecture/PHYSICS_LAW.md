...
> **数据来源**：63,963个暴力样本（近三年，阶梯筛选：2d>15% / 3d>25% / 4d>35% / 5d>60%）
...
| P50 | 2.77 | **1.96** | 2.03 |
| P75 | 6.88 | 2.90 | **7.06** |
| P90 | 15.35 | 4.33 | 16.00 |

### 系统参数（strategy_params.json V2.6.2）

```python
# 基于真实样本验证的阈值
min_avg_amount = 1.2亿      # P25覆盖75%真龙
min_volume_multiplier = 2.0  # P50=1.96x
space_gap_threshold = 7.0    # P75=7.06%
live_percentile = 0.92       # 92分位=黄金分割点
```

### 关键发现

1. **短期暴力型(2-6d)**：冷启动特征，量比要求高(2x)，换手率低
2. **长期接力型(10d)**：热启动特征，量比要求低(1.2x)，换手率高
3. **5d暴力型**：空间差中位=0%，全部在60日高点附近起爆
