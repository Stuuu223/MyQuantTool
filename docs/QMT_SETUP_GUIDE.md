...
1. **运行环境**: 
   - 本项目默认运行环境：券商 QMT + MiniQMT 极简模式 + xtquant Python 包
   - 回测/复盘阶段：MiniQMT 只需启动，无需实盘登录
   - Python 只通过本地端口连接，不得直接连 VIP 域名

2. **交易风险**: 使用实盘交易接口前，请充分测试策略
3. **权限限制**: 某些功能需要特定的交易权限
4. **数据限制**: 历史数据需要提前下载
5. **网络依赖**: 需要保持稳定的网络连接
6. **时间限制**: 某些功能只在交易时间内可用
