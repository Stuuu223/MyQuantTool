"""
全市场扫描器
Source: logic/strategies/full_market_scanner.py
    ...
    def scan_with_risk_management(self, mode='premarket', stock_list=None) -> Dict:
        ...
        try:
            ...
            for idx, candidate in enumerate(hot_pool[:sample_size]):
                ...
                # 使用数据抽象层获取资金流（P0修复：移除直接调用FundFlowAnalyzer）
                try:
                    flow_signal = self.data_provider.get_realtime_flow(code_6digit)
                    # 转换为字典格式（兼容旧代码）
                    flow_data = {
                        'stock_code': code,
                        'latest': {
                            'date': '2026-02-12',
                            'main_net_inflow': flow_signal.main_net_inflow,
                            'super_large_net': flow_signal.super_large_inflow,
                            'large_net': flow_signal.large_inflow
                        }
                    }
                except Exception as e:
                    flow_data = {'error': str(e), 'stock_code': code}
    ...
    def _level2_capital_analysis(self, candidates) -> List[dict]:
        ...
        for candidate in track(candidates, description="[green]💰 Level 2 资金流向分析..."):
            try:
                ...
                # 获取资金流向（使用数据抽象层）
                # P0修复：移除直接调用FundFlowAnalyzer，改用data_provider
                try:
                    flow_signal = self.data_provider.get_realtime_flow(code_6digit)
                    # 转换为字典格式（兼容旧代码）
                    flow_data = {
