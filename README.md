...
**公式:** `换手率 = (当前累计成交量[手] × 100 / 流通股本[股]) × 100`
**铁律:** 死亡拦截线定死为 **70%**。绝不允许爆量绞肉机入池！
...
### 2026-03-15: 流星坠毁死代码 + net_inflow假数据 Bug (V173 CTO审计)

**Bug #1: 流星坠毁死代码**
- **现象**: L544-547的`if is_micro_collapsed`分支永远不执行
- **根因**: L500已`return 0.0`，后续惩罚代码是死代码；注释写4.5%代码用3.5%
- **修复**: 选择方案A保留提前return（死刑语义），删除死代码，统一注释为3.5%
- **修复位置**: `kinetic_core_engine.py:542-547`
- **Git**: `f13a5ab`

**Bug #2: net_inflow使用全天累计额**
- **现象**: 分数随时间单调增长，09:30很低14:57很高
- **根因**: `net_inflow=current_tick['amount'] * 0.5`用的是全天累计成交额
- **影响**: 违背"买在分歧"哲学，下午假高分欺骗系统
- **修复**: 改用`true_flow_5min`真实时间切片成交额
- **修复位置**: `mock_live_runner.py:386`
- **Git**: `a3e59b0`

**Bug #3: FloatVolume单位判断不一致**
- **现象**: mock_live_runner用`<1000`数值猜测，引擎用市值法`<2亿`
- **根因**: 两套逻辑并存，违反SSOT原则
- **修复**: 统一用市值法检测单位：`est_market_cap < 2亿`则升维
- **修复位置**: `mock_live_runner.py:184-192`
- **Git**: `902c466`

**血泪教训**:
> **死代码比没代码更可怕！** 注释和代码不一致会误导维护者。时间切片数据必须用真实窗口，不能用全天累计假数据。
