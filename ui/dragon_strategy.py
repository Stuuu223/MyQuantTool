...
def render_dragon_strategy_tab(db, config):
    ...
    Args:
        ...
        config: 配置实例
    """
    st.subheader("🔥 龙头战法 - 捕捉潜在龙头股（极速版）")
    st.caption("基于财联社龙头战法精髓：快、狠、准、捕食 | 使用 Easyquotation 极速引擎")
    ...
    **龙头战法核心要点：**
    ...
    with col_scan3:
        scan_scope = st.selectbox("扫描范围", ["全市场（前N只）", "自选股"], key="scan_scope")
    with col_scan4:
    ...
    if st.session_state.get('scan_dragon', False):
        ...
        with st.spinner('正在极速扫描市场中的潜在龙头股...'):
            try:
                # 根据扫描范围获取股票列表
                if scan_scope == "自选股":
                ...
                if not stock_list:
                    ...
                    # 显示结果
                    st.success(f"扫描完成！共扫描 {len(stock_list)} 只股票，发现 {len(results)} 只符合条件股票")

                    if results:
                        # 按评分分组显示
                        strong_dragons = [s for s in results if s['score'] >= 85]
                        potential_dragons = [s for s in results if 75 <= s['score'] < 85]
                        other_dragons = [s for s in results if s['score'] < 75]
                        ...
                        if strong_dragons:
                            ...
                            for stock in strong_dragons:
                                with st.expander(f"{stock['name']} ({stock['code']}) - 评分: {stock['score']:.1f}"):
                                    ...
                                    # 添加到自选股按钮
                                    if st.button(f"➕ 添加到自选", key=f"add_dragon_{stock['code']}"):
                                        watchlist = config.get('watchlist', [])
                                        if stock['code'] not in watchlist:
                                            watchlist.append(stock['code'])
                                            config.set('watchlist', watchlist)
                                            st.success(f"已添加 {stock['name']} ({stock['code']}) 到自选股")
                                        else:
                                            st.info(f"{stock['name']} ({stock['code']}) 已在自选股中")
                        ...
                        if potential_dragons:
                            ...
                            for stock in potential_dragons:
                                with st.expander(f"{stock['name']} ({stock['code']}) - 评分: {stock['score']:.1f}"):
                                    ...
                                    # 添加到自选股按钮
                                    if st.button(f"➕ 添加到自选", key=f"add_potential_{stock['code']}"):
                                        watchlist = config.get('watchlist', [])
                                        if stock['code'] not in watchlist:
                                            watchlist.append(stock['code'])
                                            config.set('watchlist', watchlist)
