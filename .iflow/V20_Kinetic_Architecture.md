# V20 动能架构重构 - 宏微观结合四步猎杀法

**文档版本:** V20.5 极致全息·游资心法版  
**制定日期:** 2026-02-27  
**制定人:** CTO + Boss战略指导  
**执行人:** AI项目总监 + 开发团队  

---

## 一、战略背景

Boss指出当前系统三大致命问题：
1. **票太难看** - 选出的都是六国化工等死水大盘票
2. **只扫一次就停** - 没有动态巡航机制
3. **Magic Number泛滥** - 1%换手率是闭门造车，不符合A股实际

**核心战略:**  
> "避免无效的布朗运动。A股的超强流动性只体现在主升，90%的票90%的时间都在做无效的布朗运动。先粗筛出来主升趋势的票，分析动能、势能，顺势而为，资金为王，然后Rank干大哥。"

---

## 二、全局架构：宏微观四步猎杀法

### 🗡️ Phase 0: 肃清Magic Number，全面Ratio化

**问题:** 代码中大量硬编码数字（1%换手、1.5量比阈值等）  
**方案:** 所有阈值改为基于市场动态分位数和个股历史Ratio

**具体措施:**
1. **量比分位数** - 保留0.90分位动态阈值（已完成）
2. **换手率Ratio化** - `Turnover_Ratio = 今日估算全天换手率 / 5日平均换手率`
3. **全部参数Config化** - 写入strategy_params.json，零硬编码

---

### 🗡️ Phase 1: 宏观势能过滤（过滤布朗运动，只要主升）

**目标:** 只做主升浪的票，拒绝接飞刀  
**核心逻辑:** 大道至简 - 均线多头排列

**筛选条件（必须同时满足）:**
1. **趋势防线** - `MA5 >= MA10` 且 `昨日收盘价 >= MA20`
2. **资金活跃度Ratio** - `Turnover_Ratio > 1.5`（今天比平时活跃50%以上）
3. **大哥起步线** - `今日估算全天换手率 >= 5.0%`（无5%不妖股）
4. **死亡换手熔断** - `今日估算全天换手率 <= 70.0%`（【CTO铁血令】规避筹码断层）
5. **量比分位数** - `volume_ratio >= 全市场0.90分位`

**数据来源:**
- MA5/MA10/MA20: QMT本地日K数据
- 5日平均换手率: TrueDictionary
- 量比: 实时计算（时间进度加权）

---

### 🗡️ Phase 2: 动态雷达60秒极速巡航（保持池子活水）

**问题:** 当前系统只在启动时扫一次，刻舟求剑  
**方案:** 60秒一次全市场快照扫描

**实现机制:**
1. 在`_fire_control_mode`中添加`while True:`主循环
2. 记录`last_scan_time`，每60秒触发一次`_catch_up_mid_day()`
3. 动态对比新旧watchlist:
   - 新晋股票：补充Tick订阅
   - 掉队股票：取消订阅（可选）
4. 输出扫描日志：
   ```
   [CTO雷达巡航] 第N次扫描完成 | 观察池: XX只 | 新晋: X只 | 掉队: X只
   ```

**技术要点:**
- `xtdata.get_full_tick()`全市场5000只股票仅需几十毫秒
- 60秒间隔对CPU和QMT无压力
- 必须使用独立线程，避免阻塞Tick回调

---

### 🗡️ Phase 3: 构建微观物理引擎(KineticEnergyCalculator)

**目标:** 用物理思维计算Tick级动能  
**核心指标:**

1. **瞬时涨速(Velocity)**
   - 公式: `(当前Tick价格 - 60秒前Tick价格) / 昨收价 * 100%`
   - 意义: 不管-5%还是+5%，只要一分钟内拉升1.5%+就是点火

2. **动能换手(Burst Turnover)**
   - 公式: `过去60秒内累计成交量 / 流通股本 * 100%`
   - 意义: 涨速必须伴随爆量，才是主力真实扫货

3. **动能加速度(Acceleration)**
   - 逻辑: 60秒内密集千手大单 > 零星散户交易
   - 实现: Tick频率加权计算

**技术实现:**
```python
# logic/execution/kinetic_engine.py
from collections import deque
import time

class KineticEnergyCalculator:
    def __init__(self, window_seconds=60):
        self.tick_window = deque(maxlen=1000)  # 60秒Tick窗口
        
    def on_tick(self, tick_data):
        # 添加新Tick
        self.tick_window.append({
            'timestamp': time.time(),
            'price': tick_data['price'],
            'volume': tick_data['volume']
        })
        
        # 清理过期Tick
        cutoff_time = time.time() - self.window_seconds
        while self.tick_window and self.tick_window[0]['timestamp'] < cutoff_time:
            self.tick_window.popleft()
    
    def calculate_velocity(self, pre_close):
        """计算60秒涨速"""
        if len(self.tick_window) < 2:
            return 0.0
        old_price = self.tick_window[0]['price']
        new_price = self.tick_window[-1]['price']
        return (new_price - old_price) / pre_close * 100
    
    def calculate_burst_turnover(self, float_volume):
        """计算60秒动能换手"""
        total_volume = sum(t['volume'] for t in self.tick_window)
        return total_volume / float_volume * 100
```

**性能要求:**
- 严禁使用Pandas DataFrame
- 必须使用原生Python（deque）保证微秒级计算
- 每只股票独立实例，O(1)时间复杂度

---

### 🗡️ Phase 4: Tick级开火逻辑改造（消灭红盘偏见）

**目标:** 清除所有`last_price > pre_close`静态红盘代码  
**新开火条件（满足其一即可）:**

**条件A（稳健型）:**
- 宏观在主升趋势内（MA5>=MA10>=MA20）
- 动态量比 > 0.95分位
- 总换手率 > 5%

**条件B（深水炸弹/瞬间动能，Boss特批）:**
- `velocity_60s > 1.5%`（一分钟内抢筹拉升超1.5%）
- `burst_turnover_60s > 0.15%`（动能区间内爆量）
- 规避死亡换手（<70%）【CTO铁血令】

**清除清单:**
- [x] `tasks/run_live_trading_engine.py` 中的红盘限制
- [x] `logic/backtest/hot_replay_engine_v20.py` 中的红盘限制
- [x] `logic/strategies/unified_warfare_core.py` 中的红盘评分权重

---

## 三、最新架构演进 (V20.5)

### CTO终极净化 (2026-02-27)
**最新状态:** [CTO战前终极净化] 三大边缘隐患已清除

**1. MFE向上做功优化**
- 使用 `(price-low + high-open)/2` 过滤天地板
- 避免极端价格波动对MFE指标的干扰

**2. 流入比保护**
- 添加 `float_market_cap > 1000` 检查
- 流入比限制在 -50%~50% 范围内
- 防止异常值对评分系统的影响

**3. MFE一字板惩罚**
- 排序时对 MFE > 5.0 的情况进行倒扣分
- 避免缩量秒板等无意义涨停的误判

**验证结果**: 
- MFE 指标范围回归合理区间 (0.52~1.09)
- 两套看板结果100%一致

### CTO天网对齐 (2026-02-26)
**核心改进:** 
- 修复MFE量纲问题
- 统一数据源 (SSOT - Single Source of Truth)
- ATR计算中的pre_close列缺失问题修复

---

## 四、参数配置（strategy_params.json）

```json
{
  "kinetic_radar": {
    "_description": "V20动能雷达参数 - 资金为王顺势而为",
    
    "scan_interval_seconds": 60,
    
    "macro_trend_filter": {
      "ma5_above_ma10": true,
      "close_above_ma20": true,
      "_note": "大道至简：只做多头排列主升票"
    },
    
    "turnover_thresholds": {
      "ratio_to_5d_avg": 1.5,
      "absolute_min_percent": 5.0,
      "absolute_max_percent": 70.0,
      "_note": "5%起步是妖股，70%死亡换手规避【CTO铁血令】"
    },
    
    "volume_ratio_percentile": 0.90,
    
    "kinetic_fire_triggers": {
      "velocity_60s_min": 1.5,
      "burst_turnover_60s_min": 0.15,
      "_note": "一分钟拉升1.5%且爆量0.15%即触发"
    }
  }
}
```

---

## 五、执行纪律与验收标准

### 总监红线:
1. **分Phase执行** - 每完成一个Phase必须Git提交
2. **真实测试** - 每完成一个Phase运行`python main.py live --dry-run`
3. **日志验收** - 必须看到：
   - `[CTO雷达巡航] 第N次扫描完成`
   - `观察池: XX只`（应该是活跃主升浪票，不是死水）
   - `✅ 成功接收并解析 Tick`

### Boss验收标准:
1. 选出的票不再是六国化工等死水票
2. 系统是活的，60秒扫描一次
3. 能看到动能计算和开火逻辑工作

---

## 六、当前进度追踪

| Phase | 任务 | 状态 | 备注 |
|-------|------|------|------|
| Phase 0 | 肃清Magic Number | ✅ 已完成 | 全部Ratio化 |
| Phase 1 | 宏观势能过滤 | ✅ 已完成 | MA5/MA10/MA20+5%换手底线 |
| Phase 2 | 60秒极速巡航 | ✅ 已完成 | 动态雷达活水 |
| Phase 3 | KineticEngine | ✅ 已完成 | 60秒滑动窗口 |
| Phase 4 | 开火逻辑改造 | ✅ 已完成 | 清除红盘偏见 |
| Phase 5 | MFE优化 | ✅ 已完成 | 天地板过滤+一字板惩罚 |
| Phase 6 | 数据源统一 | ✅ 已完成 | SSOT原则 |

---

**下一步行动:** 系统已进入稳定运行状态，重点优化回测精度和实盘表现。