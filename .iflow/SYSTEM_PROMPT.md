...
### 版本更新规则
- **主版本** (V9 → V10): 重大架构变更、不兼容更新
- **次版本** (V9.4 → V9.5): 新增功能、重要改进
- **修订号** (V9.4.5 → V9.4.6): Bug 修复、小改进

### 版本变更记录
每次版本更新必须同步更新以下文件：
1. `logic/logger.py` - 更新 `__version__ = "V9.4.5"`
2. `PROJECT_STRUCTURE.md` - 更新项目概述中的版本号
3. `.iflow/SYSTEM_PROMPT.md` - 更新本文件的版本号
4. `docs/dev/VERSION_HISTORY.md` - 记录详细变更日志
...
- [ ] 所有函数都有 docstring（Google 风格）
- [ ] 使用 Python 3.10 类型注解（`list[str]` 不是 `List[str]`）
- [ ] 删除所有调试代码（`print`, `pdb.set_trace()`）
- [ ] 删除所有注释掉的代码（dead code）
- [ ] 变量命名清晰（避免 `tmp`, `data1`, `x`）

### ✅ 功能完整性
- [ ] 功能符合需求描述
- [ ] 边界情况已处理（空数据、异常值、网络超时）
- [ ] 错误处理完善（try-except + 日志记录）
- [ ] 添加了必要的输入验证

### ✅ 性能和安全
- [ ] 没有硬编码的敏感信息（API Key、密码）
- [ ] API 调用符合速率限制（参考 docs/tech/速率限制说明.md）
- [ ] 内存使用合理（大数据使用生成器/迭代器）

### ✅ 测试
- [ ] 单元测试已编写并通过
- [ ] 使用真实数据验证（300997/603697 案例）

### ✅ 文档
- [ ] 更新了相关文档（如有变更）
- [ ] 更新了版本历史（docs/dev/VERSION_HISTORY.md）
