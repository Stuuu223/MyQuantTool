...
- **版本号**：V9.4.8（紧急修复版）
- **Git提交**：563656b
- **提交信息**：hotfix: 修复AkShare数据排序BUG（紧急）

### 🚨 V9.4.8 紧急修复（2026-02-09 15:35）

#### Bug描述
- **问题**：price_3d_change计算出现异常值
  - 宝馨科技：+110%（不可能！）
  - 鸿路钢构：-65.54%（不可能！）
- **根本原因**：AkShare返回的DataFrame未按日期排序
  - `iloc[-4]`取到的是2025年1月1日的数据
  - 实际计算的是13个月的涨幅，而非3日涨幅
- **影响范围**：
  - 诱多检测完全失效
  - 将长期牛股误判为短期急涨
  - 可能导致错误的交易决策
...
if df is not None and not df.empty:
    df.sort_values('日期', ascending=True, inplace=True)  # 🔥 关键修复
```

#### 预期效果
- ✅ price_3d_change在合理范围内（-20% 到 +33%）
- ✅ 3日涨停股的price_3d_change应≈33%
- ✅ 不会再出现+110%这种不可能的值
...
- **修复1**: price_3d_change缺失问题
  ...
  - ⚠️ **注意**：V9.4.7的验证有BUG（排序问题），已在V9.4.8修复
...
- 保存版本：V9.4.8（紧急修复版）
- Git提交：563656b
- **最后更新**：2026-02-09 15:35（修复AkShare排序BUG）

## 🚨 已知问题
- **网络连接**：AkShare API偶尔超时，已增加重试机制
- **数据排序**：V9.4.8已修复，需明早验证
- **T+1验证**：样本不足（仅3个），需继续积累至少17个样本