...
class GlobalFilterGateway:
    ...
    def apply_boss_two_dimensional_filters(
        ...
        """
        【Boss二维铁网 - V20.3红线�?+ 早盘降阈 + P0修复 + CTO疑点修复】应用统一过滤逻辑
        
        【CTO红线改造说明�?        - 完全删除MA5/MA10/MA20均线判定（权力下放给战法Detector�?        - 只做0/1生死判定，无打分机制
        - 死亡换手拦截统一�?=70%（新股在stock_filter第一道被过滤，不会到这里�?        - 甜点位标�?8%-15%)，仅注入tag，绝不加�?        【CTO紧急修复】早盘降�? 09:30-09:45阈值降�?0%
        【Boss P0修复】ATR缺数据保留NaN，不伪装成低能�?        【CTO疑点修复�?        - 疑点#1: 死亡换手 `>` �?`>=`
        - 疑点#3: 单位自适应 `<= 1.0` �?`< 1.0`
        - 疑点#4: ATR默认�?`0.05` �?`None`
        - 疑点#5: 早盘时区修复 UTC+8
        ...
        Args:
            df: 输入DataFrame，必须包含以下列:
                ...
                - volume_ratio: 量比倍数
                - turnover_rate: 换手�?%)
            config_manager: 配置管理�?            true_dict: TrueDictionary实例（用于获取ATR数据�?            context: 调用上下文（用于日志区分�?realtime"/"replay"/"backtest"�?            
        Returns:
            Tuple[filtered_df, stats_dict]: 
                - filtered_df: 过滤后的DataFrame（新�?tag'列标记甜点位�?atr_ratio'列保留NaN�?                - stats_dict: 过滤统计信息（包含atr_data_missing计数�?        """
        if df.empty:
            logger.warning(f"[{context}] 输入数据为空，跳过过�?)
            return df, {
        ...
        if is_early:
            logger.info(f"  �?[早盘降阈] 检测到早盘时间窗口(09:30-09:45)，阈值降�?0%，捕捉意愿度")
        
        # 【从Config读取所有阈�?- SSOT原则�?        try:
            # 读取基础阈�?            base_min_volume = config_manager.get('live_sniper.min_volume_multiplier')
            base_min_turnover = config_manager.get('live_sniper.min_active_turnover_rate')  # 5%
            sweet_spot_min = config_manager.get('live_sniper.sweet_spot_min', 8.0)  # 甜点位下�?            sweet_spot_max = config_manager.get('live_sniper.sweet_spot_max', 15.0)  # 甜点位上�?            
            # 验证读取成功
            if None in [base_min_volume, base_min_turnover]:
                raise ValueError("核心配置缺失")
            
            # 【CTO紧急修复】应用早盘缩放因�?            min_volume_multiplier = base_min_volume * scale_factor
            ...
            raise RuntimeError("系统拒绝启动：缺少核心过滤配�?)
        
        logger.info(f"[{context}] Boss三维铁网启动(V20.3物理重铸�?早盘降阈+P0修复+CTO疑点修复) | 输入: {original_count}�?| "
        ...
        if 'turnover_rate' in df.columns:
            ...
            # 【疑�?3修复】换手率单位自适应：`<= 1.0` �?`< 1.0`，避免max=1.0%被误�?            if df['turnover_rate'].max() < 1.0:
                logger.info(f"  🔧 检测到换手率数据为小数形式，自动转换为百分�?)
                df['turnover_rate'] = df['turnover_rate'] * 100.0
        
        # 【CTO类型安全】volume_ratio也强制转�?        if 'volume_ratio' in df.columns:
        ...
        # 【Boss裁决�?50%是游资出货完毕红线，150.0%本身就应拦截
        if 'turnover_rate' in df.columns:
            death_mask = df['turnover_rate'] >= GlobalFilterGateway.DEATH_TURNOVER_THRESHOLD  # 【疑�?1修复�?            death_count = death_mask.sum()
            if death_count > 0:
                ...
                # 从df中删除死亡换手股�?                df = df[~death_mask].copy()
        
        # ========== 第一维：量能网（0/1判定，无打分�?==========
        # 量比 >= 配置倍数（如3.0倍）- 纯生死判�?        if 'volume_ratio' in df.columns:
            ...
            logger.info(f"  🔹 量能�? {volume_after}/{volume_before}只通过 (量比>={min_volume_multiplier:.2f}x)�?/1判定{'，早盘降�? if is_early else ''}�?)
            # 【物理探针】打印被淘汰的阈值边界信�?            if volume_rejected > 0:
                min_ratio = df['volume_ratio'].min() if len(df) > 0 else 0
                max_ratio = df['volume_ratio'].max() if len(df) > 0 else 0
                logger.info(f"     📊 通过者量比范�? {min_ratio:.2f}x ~ {max_ratio:.2f}x")
                logger.info(f"     🚫 淘汰: {volume_rejected}只因量比<{min_volume_multiplier:.2f}x")
        
        # ========== 第二维：换手网（0/1判定，无打分，只检查下限） ==========
        # 换手�?>= min_turnover (5%) - 纯生死判�?        # 上限已在死亡换手拦截处理，不重复过滤
        if 'turnover_rate' in df.columns:
            ...
            logger.info(f"  🔹 换手�? {turnover_after}/{turnover_before}只通过 (换手>={min_turnover:.2f}%)�?/1判定{'，早盘降�? if is_early else ''}�?)
        
        # ========== 第三维：ATR势垒网（可配置：仅记�?硬过滤，P0修复+疑点#4修复�?=========
        # 【ATR阈值说明�?        # - 当前采用 atr_ratio >= 1.8x（研究报告推荐值）
        # - 研究样本�?026-02-27�?026-03-02�?天）
        # - 结论：atr_ratio >= 1.8时，涨停概率提升3.2�?        # - TODO: 后续需三个月样本重新估此参�?        #
        # 【物理学意义�?        # - ATR = Average True Range = 真实波幅均�?        # - atr_ratio = 今日波幅 / 20日平均真实波�?        # - atr_ratio > 1.8 表示今日波动超过历史平均�?.8�?        # - 高能�?= 高波�?= 资金活跃度高 = 起爆概率�?        #
        ...
        # - 区分"缺数�?�?真低能�?，用于回测统�?        #
        # 【疑�?4修复�?2026-03-04 CTO审计)
        # - atr_20d 默认�?`0.05` �?`None`
        ...
        if atr_filter_enabled and true_dict and 'stock_code' in df.columns and len(df) > 0:
            try:
                # 【疑�?4修复】获取ATR数据�?0日平均真实波幅），默认None而非0.05
                ...
                if 'high' in df.columns and 'low' in df.columns:
                    # 【CTO修复】使用numpy.where处理NaN，缺失数据用1.0默认�?                    import numpy as np
                    
                    # prev_close�?或NaN时，today_tr=0（无法计算）
                    ...
                    # atr_20d�?或NaN时，atr_ratio=1.0（默认市场平均水平，放行！）
                    df['atr_ratio'] = np.where(
                        (df['atr_20d'].notna()) & (df['atr_20d'] > 0) & (df['today_tr'] > 0),
                        df['today_tr'] / df['atr_20d'],
                        1.0  # 【CTO修复】缺失数据默�?.0，代表市场平均水平，放行�?                    )
                    
                    # 【CTO修复】简化警告：只打印缺失数量，不打印具体代�?                    if missing_count > 0:
                        logger.info(f"  🔹 ATR势垒�? {missing_count}只股票数据缺失，使用默认�?.0x（市场平均水平）")
                    ...
                    # 根据模式决定是否硬过�?                    if atr_filter_mode == 'hard_filter':
                        # 硬过滤：拦截低能态股票（NaN单独统计�?数据缺失，无法判�?�?                        mask_atr = df['atr_ratio'] >= atr_ratio_min
                        df = df[mask_atr].copy()
                        atr_after = len(df)
                        stats["atr_filtered"] = atr_rejected
                        
                        logger.info(f"  🔹 ATR势垒�? {atr_after}/{atr_before}只通过 (ATR比率>={atr_ratio_min}x)【硬过滤模式�?)
                        ...
                        logger.info(f"  🔹 ATR势垒�? {atr_pass_count}/{atr_before}只达�?(ATR比率>={atr_ratio_min}x)【仅记录模式，不拦截�?)
                        if atr_rejected > 0:
                            logger.info(f"     📊 未达�? {atr_rejected}只ATR比率<{atr_ratio_min}x（待回测验证后再决定是否拦截�?)
                else:
                    logger.warning(f"  ⚠️ ATR势垒�? 缺少high/low列，跳过ATR计算")
                    df['atr_ratio'] = float('nan')
                    
            except Exception as e:
                logger.warning(f"  ⚠️ ATR势垒网计算失�? {e}，跳过ATR计算")
                df['atr_ratio'] = float('nan')
        else:
            if not atr_filter_enabled:
                logger.info(f"  🔹 ATR势垒�? 已禁�?(atr_filter_enabled=False)")
            elif 'stock_code' not in df.columns:
                logger.warning(f"  ⚠️ ATR势垒�? 缺少stock_code列，跳过ATR计算")
            elif len(df) == 0:
                logger.info(f"  🔹 ATR势垒�? 输入为空，跳�?)
            else:
                logger.warning(f"  ⚠️ ATR势垒�? 缺少true_dict，跳过ATR计算")
            df['atr_ratio'] = float('nan')
        ...
        if 'turnover_rate' in df.columns and len(df) > 0:
            ...
            # 初始化tag列为空字符串
            df['tag'] = ''
            # 对甜点位股票注入tag（仅标记，绝不加分！�?            df.loc[sweet_mask, 'tag'] = '换手甜点'
            
            if sweet_count > 0:
                logger.info(f"  🍰 甜点位标�? {sweet_count}只被标记�?换手甜点' (换手{sweet_spot_min}%~{sweet_spot_max}%)"
                           f"【仅注入tag，绝不加分！�?)
        
        # ========== 【CTO红线】趋势网已完全删�?==========
        # MA5/MA10/MA20判定已删除，权力下放给战法Detector�?        # 不再进行任何均线相关的过滤或打分
        logger.info(f"  �?趋势�? 【已删除】均线判定权力下放给战法Detector")
        
        stats["output"] = len(df)
        stats["filter_rate"] = f"{len(df)/original_count*100:.1f}%" if original_count > 0 else "0%"
        
        logger.info(f"[{context}] Boss二维铁网完成 | 输出: {len(df)}�?| 通过�? {stats['filter_rate']} | "
                   f"死亡换手拦截: {stats['death_turnover_blocked']}�?| ATR数据缺失: {stats['atr_data_missing']}�?| "
