...
class KineticCoreEngine:
    ...
    def _load_config(self) -> None:
        ...
        self.time_decay_tail_trap: float = decay_config.get('tail_trap', 0.5)  # 【CTO M4修复】尾盘腰斩
    ...
    def calculate_true_dragon_score(
        ...
        # 【CTO V160 物理学修正：情绪透支阻尼】
        # 根据 V158 回归分析（8014样本），昨日量比与今日开盘溢价呈绝对负相关！
        # 量比越高，意味着情绪越透支，次日接盘侠越少。
        # 物理真相：昨日量比95th(8.2x)次日溢价仅+1.19%，50th(1.0x)溢价+4.67%
        
        if yesterday_vol_ratio > 1.0:
            # 透支效应：量比越高，乘数越低（负相关）
            # 使用减法产生阻尼，量比10x时乘数约0.5被保底拦截
