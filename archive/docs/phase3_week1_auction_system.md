...
## 🎯 开发目?
构建一?*竞价快照采集、存储、回放、检?*的完整系统，用于?
1. **竞价数据积累**：每个交易日09:25采集全市场竞价数?2. **诱多模式识别**：检测“竞价高开+开盘砸盘”等诱多模式
3. **历史回测**：回放历史竞价快照，验证策略有效?
---
...
**核心功能**?- 每个交易?9:25采集全市场竞价数?- 双重存储：SQLite + Redis（兼顾持久化和实时查询）
- 支持单日采集和批量历史数据采?- 自动跳过周末，智能进度显?
**使用方法**?```bash
# 1. 采集今日竞价快照（每天交易后执行?python tasks/collect_auction_snapshot.py
...
**数据结构**?```python
{
    ...
    'auction_price': 15.50,
    'auction_volume': 500000,  # 竞价成交量（手）
    'auction_amount': 7750000,  # 竞价成交额（元）
    'auction_change': 0.0333,  # 竞价涨幅 (3.33%)
    'volume_ratio': 2.5,  # 量比
...
- 竞价放量（量?> 1.5?
**逻辑**? 
庄家在竞价阶段大量买入，制造高开幻象。开盘后立即砸盘，散户被套?
**历史案例**?- **300997（欢乐家?*?026-01-29 竞价高开+3.33%，开?分钟砸盘-2.9%
...
- 竞价涨幅适中?-5%?
**逻辑**? 
竞价阶段大量买入，但开盘后尾盘回落，说明接盘无力?
**历史案例**?- **603697（有友食品）**?026-02-02 竞价爆量2.5倍，尾盘回落-1.5%
...
if result.trap_type != TrapType.NORMAL:
    print(f"⚠️ 发现诱多: {result.trap_type.value}")
...
**核心功能**?- 回放任意历史日期的竞价快?- 结合开盘后5分钟K线数?- 自动调用诱多检测器
- 输出美观的表格报?
**使用方法**?```bash
# 1. 回放指定日期的竞价快?python tasks/replay_auction_snapshot.py --date 2026-02-10
...
**筛选条?*?- `all`：所有股票（默认?- `high_open`：竞价高开 > 3%
- `low_open`：竞价低开 < -3%
- `high_volume`：量?> 2.0
...
+------------+----------+----------+----------+------+----------+--------------------+-----------+--------+------------------------+
| 代码       | 名称     | 竞价涨幅 | 开盘涨?| 量比 | 尾盘回落 | 诱多类型           | 风险级别 | 置信?| 信号                   |
+============+==========+==========+==========+======+==========+====================+===========+========+========================+
| 300997.SZ  | 欢乐?  | +3.33%   | -2.90%   | 2.5x | 2.90%    | AUC_HIGH_OPEN_DUMP | 🔴 ?   | 90%    | 竞价高开, 开?分钟砸盘 |
+------------+----------+----------+----------+------+----------+--------------------+-----------+--------+------------------------+
...
if result.trap_type != TrapType.NORMAL:
    # 高风险诱多，不参?    if result.risk_level == RiskLevel.HIGH:
        logger.warning(f"⚠️ {code} 检测到高风险诱多，不参?)
```
...
1. **实时监控面板**?   - Streamlit 实时显示竞价数据
   - 可视化诱多检测结?
2. **多维度竞价分?*?   - 资金流向分析（机?vs 散户?   - 买卖单分布分?   - 竞价成交金额分布

3. **智能预警**?   - 实时预警高风险诡?   - 微信/邮件推?
### Phase3 ?周（计划中）

1. **机器学习模型**?   - 基于历史数据训练诱多检测模?   - 提高检测精度和置信?
2. **策略回测框架**?   - 集成到回测框?   - 评估竞价策略有效?
---

## 📝 更新日志

### 2026-02-11 - V1.0 ?开发完?
**新增功能**?- ?竞价快照采集脚本 (`tasks/collect_auction_snapshot.py`)
- ?竞价诱多检测器 (`logic/auction_trap_detector.py`)
- ?竞价快照回放?(`tasks/replay_auction_snapshot.py`)
- ?SQLite + Redis 双重存储
- ?3种竞价诱多模式检?
...
Phase3?周开发已完成核心功能?
1. ?**数据采集**：可每日自动采集全市场竞价数?2. ?**模式检?*：可检?种诱多模式，输出风险级别
