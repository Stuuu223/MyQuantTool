"""K线分析仓表板 - 实时技术面监控 (Real Data Integration v2.0)

改造完成:
✅ 自选股行情支持 DataManager / Demo 双模式
✅ K线图支持真实数据接入
✅ 统一错误处理
"""
...
try:
    ...
    logging.warning("❌ DataManager 不可用，K线仓表板使用 Demo 数据")
...
with st.sidebar:
    ...
    if REAL_DATA_AVAILABLE:
        ...
        st.warning("⚠️ 当前使用 Demo 模拟数据")

    data_source = st.selectbox(
        "数据源",
        ["Demo 模拟数据", "DataManager 实时数据"],
        index=1 if REAL_DATA_AVAILABLE else 0
    )
...
def get_quote_data(codes, source: str) -> pd.DataFrame | None:
    """获取自选股行情数据"""
    if source == "DataManager 实时数据" and REAL_DATA_AVAILABLE and DataManager:
        try:
            dm = DataManager()
            quote_list = []
            for code in codes:
                try:
                    # TODO: 真实行情接口挂接点: dm.get_realtime_quote(code)
                    record = {
                        '代码': code,
                        '名称': f'股票{code}',
                    ...
                    logging.warning(f"获取 {code} 行情失败: {e}")
...
def get_kline_data(main_code: str, frame: str) -> pd.DataFrame:
    """获取 K线 数据 (预留真实接口)"""
    try:
        if REAL_DATA_AVAILABLE and DataManager:
            dm = DataManager()
            # TODO: 对接真实 K 线接口，根据 frame 选择周期
            # return dm.get_kline(main_code, period=frame)
    except Exception as e:
        logging.warning(f"获取 K线 数据失败: {e}")
    # Demo K线
...
with tab1:
    ...
    quote_data = get_quote_data(watch_list, data_source)
    if quote_data is not None and len(quote_data) > 0:
        st.dataframe(quote_data, use_container_width=True, hide_index=True)
    else:
        st.warning("🕔 没有选中任何股票或数据加载失败。请在侧边栏选择股票。")

    st.divider()

    st.subheader("📈 K线图表")

    main_code = watch_list[0] if watch_list else '600519'
    kline_df = get_kline_data(main_code, time_frame)
    ...
    fig.update_layout(
        title=f"{main_code} K线走势（{time_frame}）",
        yaxis_title='价格',
