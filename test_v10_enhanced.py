...
def test_concept_map():
    ...
    try:
        ...
        # 检查概念映射是否加载
        if dm.concept_map:
            print(f"✅ 概念映射加载成功，共 {len(dm.concept_map)} 只股票")
            
            # 测试几只股票的概念获取
            test_stocks = ['600519', '000001', '300750']
            
            for code in test_stocks:
                concepts_data = dm.get_stock_concepts(code)
                print(f"\n股票 {code}:")
                print(f"  - 行业: {concepts_data['industry']}")
                print(f"  - 概念: {', '.join(concepts_data['concepts']) if concepts_data['concepts'] else '无'}")
            
            return True
        else:
            print("⚠️  概念映射为空，请运行 tools/update_concepts.py 生成概念数据")
            return True  # 不算失败，只是需要更新数据
...
def test_ai_context_optimization():
    ...
    print("\n" + "="*60)
    print("测试 3: AI Context 优化（Token 瘦身）")
    print("="*60)
...
def run_all_tests():
    """运行所有测试"""
    print("\n" + "="*60)
    print("V10.0 增强功能测试开始")
    print("="*60)
    print(f"测试时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    results = {
        '概念映射功能': test_concept_map(),
