...
# -*- coding: utf-8 -*-
"""
150% 死亡换手合理性验证脚本（可复核版�?+ QMT连接修复�?
【目标�?验证 150% 作为"游资出货完毕红线"的统计合理性�?
【严禁�?- 不允许声�?已执�?已结�?但不给出 data/validation 下输出文�?- 不允�?magic number 兜底伪造数据（缺数据必须显式NA�?"""
...
class ValidationConfig:
    ...
    bucket_high: float = 50.0  # �߻�����
    bucket_death: float = 70.0  # ���������ߣ�configͳһ��
...
class QMTDailyKlineProvider(DailyKlineProvider):
    ...
    def __init__(self):
        try:
            ...
            raise RuntimeError("未安�?xtquant，无法读�?QMT 数据")
    
    def get_daily(self, stock_code: str, start: datetime, end: datetime) -> pd.DataFrame:
        """
        读取日线数据（复�?TrueDictionary 的逻辑�?        
        Returns:
        ...
        # 1. 获取日线数据（time, close, volume, amount...�?        try:
        ...
        if data_dict is None or stock_code not in data_dict:
            if self._debug_count < 3:
                self._debug_logs.append(f"[{stock_code}] data_dict为空或无此股�?)
                self._debug_count += 1
            return pd.DataFrame()
        ...
        df = df.copy()
        # 【修复】xtdata返回的time是毫秒时间戳，需要转成日�?        if 'time' in df.columns:
            # 毫秒时间戳转日期
        ...
        try:
            # 【修复】is_sync=False 改为 False（QMT API不接受关键字参数�?            detail = self.xtdata.get_instrument_detail(stock_code, False)
            if detail is None:
        ...
        # 3. 计算换手率（QMT volume单位是手，FloatVolume单位是股�?        # turnover_rate = volume(�? * 100(�?�? / FloatVolume(�? * 100(%)
        df['turnover_rate'] = df['volume'] * 100.0 / float_volume * 100.0
        
        if self._debug_count < 3:
            self._debug_logs.append(
                f"[{stock_code}] 成功: {len(df)}�? "
...
def discover_stock_codes_from_datadir(qmt_userdata_path: str) -> list:
    """�?datadir/SZ/86400 �?datadir/SH/86400 目录扫描股票文件�?""
...
def run_validation(provider: DailyKlineProvider, cfg: ValidationConfig) -> None:
    ...
    if not stock_codes:
        raise RuntimeError("未在 datadir/SZ|SH/86400 发现股票文件，检�?QMT_USERDATA_PATH 是否正确�?)
    
    print(f"发现 {len(stock_codes)} 只股�?)
    ...
    for i, code in enumerate(stock_codes, 1):
        ...
        # 调试：记录前10个有效样本的换手率范�?        if debug_sample_count < 10 and 'turnover_rate' in df.columns:
        ...
        # 调试：检查日期解析结�?        if debug_sample_count < 3:
        ...
        # 只保�?forward return 有效的事�?        for n in cfg.forward_days:
    ...
    for s in debug_turnover_samples:
        print(f"  {s}")
    print(f"【调试】跳过统�? 无数�?{skipped_no_data}, 历史不足={skipped_short_hist}, 字段缺失={skipped_no_turnover}")
    print("-" * 70)
    
    if not events:
        print("\n" + "=" * 70)
        print("错误: 没有收集到任何事件样本（换手>=70%�?)
        print("请检�?provider 是否正确提供 turnover_rate")
        print("=" * 70)
        return
    ...
    # 事件明细输出
    csv_path = VALIDATION_DIR / "death_turnover_70_events.csv"
    out_df.to_csv(csv_path, index=False, encoding="utf-8-sig")
    
    # 分桶统计
    summary_lines = []
    summary_lines.append("=" * 70)
    summary_lines.append("死亡换手率验证 - 分桶统计摘要")
    summary_lines.append("=" * 70)
    summary_lines.append(f"生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    summary_lines.append(f"事件样本�?换手>={cfg.bucket_min:.0f}%): {len(out_df)}")
    ...
    for b in buckets:
        ...
        summary_lines.append(f"[{b}] 样本�? {len(dfb)}")
        if dfb.empty:
            summary_lines.append("")
            continue
    ...
    print("�?已输出事件明�?", csv_path)
    print("�?已输出统计摘�?", txt_path)
